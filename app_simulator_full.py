...
def render_hero() -> None:
    player = get_player()
    player_line = f"プレイヤー：{player.get('player_name')}" if player else "プレイヤー未設定"
    st.markdown(
        f"""
        <div class="hero">
            <h1>{APP_TITLE}</h1>
...
def screen_login() -> None:
    ...
    with col2:
        if st.button("ゲストで開始"):
            player = load_player("guest")
            set_player(player)
            go("home")

    st.info("ローカルでは players フォルダに履歴JSONを保存します。Streamlit Cloudでは永続保存が保証されないため、正式運用では外部DB化が必要です。")
